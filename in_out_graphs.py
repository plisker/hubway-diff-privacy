...
class GraphTrips(object):
	...
	def __init__(self, raw=True, filename="", file=""):
...
def main():
	...
	analyzeData.startProgress("Processing data")

	analyzeData.analysis()

	in_date_set = set(analyzeData.in_dates) # Create set to get number of unique dates for the sake of creating averages
	out_date_set = set(analyzeData.out_dates)

	analyzeData.average_matrix(analyzeData.in_matrix, len(in_date_set))
	analyzeData.average_matrix(analyzeData.out_matrix, len(out_date_set))

	analyzeData.endProgress() # Finish progress bar

	in_station_set = set(analyzeData.in_stations_represented)
	out_station_set = set(analyzeData.out_stations_represented)
